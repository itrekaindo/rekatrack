<?php if(session('success') || session('error') || session('warning') || session('info')): ?>
  ...
  <div
    x-data="{ show: true, type: '<?php echo e($type); ?>', message: '<?php echo e($message); ?>' }"
    x-show="show"
...
<?php if($errors->any()): ?>
  <div
    x-data="{ show: true, type: 'error', message: <?php echo \Illuminate\Support\Js::from($errors->first())->toHtml() ?> }"
    x-show="show"
    x-init="setTimeout(() => show = false, 5000)"
    :class="{
    'border-error-500 bg-error-50 dark:border-error-500/30 dark:bg-error-500/15': type === 'error',
...
<?php endif; ?><?php /**PATH /home/server/Reka/current-rekatrack/current/resources/views/partials/alert/alert.blade.php ENDPATH**/ ?>